...
const InputSelect = ({
  ...
  return (
    <div className="relative h-15 min-w-[200px]">
      <select
        id={id}
        defaultValue={defaultvalue}
        placeholder=" "
        {...register(name, isValidationOn ? validation : "")}
        className="peer h-full w-full rounded-[7px] border border-blue-gray-200 border-t-transparent bg-transparent px-3 py-2.5 text-sm outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-primary focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50"
      >
        {options?.map((option) => (
